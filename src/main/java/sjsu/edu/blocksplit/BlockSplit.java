...
public class BlockSplit {
	...
	public static void main(String[] args) throws IOException {
		...
		SparkConf conf = new SparkConf().setAppName("BlockSplit");
		...
		JavaPairRDD<String, String> wordToCountMap1 = lines1
		...
		JavaPairRDD<String, String> wordCounts1 = wordToCountMap1.reduceByKey(new Function2<String, String, String>() {
						...
						for (int i = 1; i < key.length; i++) {
							if (!key[i].isEmpty()) {
								...
								if (percentagecal > empcp) {
									results.add(new Tuple2<String, String>("0." + blckindex + ".*", key[0] + "{"
											+ culusterSignature.get("Ignore_" + key[0]) + "}"));
								} else {

									results.add(new Tuple2<String, String>(task.partitionId() + "." + blckindex + "."
											+ 0, key[0] + "{" + culusterSignature.get("Ignore_" + key[0]) + "}"));
									String x = key[0] + "hiphen" + task.partitionId();
									results.add(new Tuple2<String, String>("S." + blckindex + ".0X1", x + "{"
											+ culusterSignature.get("Ignore_" + key[0]) + "}"));
								}
							}
						}
		...
		JavaPairRDD<String, String> wordCounts3 = wordToCountMap3.reduceByKey(new Function2<String, String, String>() {
			public String call(String first, String second) throws Exception {
				...
				for (int j = (i + 1); j < dp.length; j++) {
					...
					if (eleminatePair == true) {
						...
						if (((dp[i].contains("hiphen0")) && (dp[j].contains("hiphen0")))) {
							...
							if (similar >= 0.7) {
								System.out.println("=============================================================");
								System.out.println("[" + xb + "," + xc + "] has " + similar + " Similarity");
								simcount = simcount + 1;
								System.out.println("=============================================================");
		...
		System.out.println("has total relavant entities" + simcount);
		System.out.println("has total number of entities" + totalent);
